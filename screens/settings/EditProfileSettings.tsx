...
interface EditProfileSettingsProps {
  currentUser: User;
  onUpdateProfile: (userId: number, updates: Partial<Pick<User, 'name' | 'username' | 'bio' | 'location' | 'avatar'>>) => void;
  onBack: () => void;
}

const EditProfileSettings: React.FC<EditProfileSettingsProps> = ({ currentUser, onUpdateProfile, onBack }) => {
  ...
  // FIX: Add useEffect with a cleanup function to revoke the object URL.
  ...
  return (
    <div className="absolute inset-0 bg-white z-40 flex flex-col">
      ...
      <main className="flex-grow overflow-y-auto p-4 space-y-6">
        <div className="flex flex-col items-center">
            <div className="relative w-24 h-24">
                <img src={avatarPreview || currentUser.avatar} alt="Profile" className="w-24 h-24 rounded-full object-cover" />
                <button
      ...
      <footer className="p-4 border-t border-gray-100">
        <button onClick={handleSaveChanges} disabled={!isChanged} className="w-full py-2.5 bg-violet-600 text-white rounded-full font-semibold hover:bg-violet-700 disabled:bg-violet-300">
            Save Changes
        </button>
      </footer>
    </div>
