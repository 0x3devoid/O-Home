...
const NotificationsScreen: React.FC<NotificationsScreenProps> = ({ notifications, onBack, onNotificationClick }) => {
  return (
    <div className="flex flex-col h-full bg-white w-full max-w-2xl mx-auto md:border-x md:border-gray-200 lg:max-w-3xl">
      ...
      <main className="flex-grow overflow-y-auto no-scrollbar">
        {notifications.length > 0 ? (
            <div className="divide-y divide-gray-100">
                {notifications.map(notification => (
                    <button 
                        ...
                        className={`w-full text-left flex items-start space-x-4 p-4 transition-colors hover:bg-gray-50 focus:outline-none focus:bg-gray-50 ${!notification.read ? 'bg-violet-50/50' : 'bg-white'}`}
                    >
            ...
            <div className="flex flex-col items-center justify-center h-full p-8 text-center">
                ...
                <p className="text-gray-500 max-w-sm">Important updates about your activity on edQorta will appear here.</p>
            </div>
        )}
      </main>
    </div>
