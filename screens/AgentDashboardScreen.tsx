...
import { User, Property, Conversation, ScheduledTour } from '../types';
import { ClipboardListIcon, MessageCircleIcon, CheckCircleIcon, ClockIcon, CashIcon, GiftIcon, CalendarIcon } from '../components/Icons';
...
interface AgentDashboardScreenProps {
    ...
    onConfirmTour: (tourId: number, time: string) => void;
}
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-lg p-3">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                <img src={tour.property.images[0]} alt={tour.property.location} className="w-full sm:w-24 h-24 object-cover rounded-md" />
                <div className="flex-1">
                    <p className="font-bold text-gray-800">{tour.property.location}</p>
                    <div className="flex items-center gap-2 mt-2">
                        <img src={tour.renter.avatar} alt={tour.renter.name} className="w-6 h-6 rounded-full" />
                        <div>
...
const AgentDashboardScreen: React.FC<AgentDashboardScreenProps> = ({ currentUser, properties, conversations, tours, onNavigateToChat, onConfirmTour }) => {
                ...
                <main className="flex-grow overflow-y-auto no-scrollbar p-4 space-y-8">
                        ...
                        <div className="mt-4 space-y-3">
                           {pendingTours.length > 0 ? (
                                pendingTours.map(tour => (
                                    <TourRequestCard key={tour.id} tour={tour} onConfirm={(time) => onConfirmTour(tour.id, time)} />
                                ))
                            ) : (
                    ...
                    <section>
                        ...
                        <p className="text-sm text-gray-500">These are verification bounties you have accepted.</p>
                         <div className="mt-4 space-y-3">
                           {/* Re-using TourRequestCard structure for simplicity */}
                           {pendingTasks.length > 0 ? (
                               pendingTasks.map(task => (
                                   <div key={task.id} className="bg-white border border-gray-200 rounded-lg p-3 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                       <img src={task.images[0]} alt={task.location} className="w-full sm:w-24 h-24 object-cover rounded-md" />
                                       <div className="flex-1">
                                           <p className="font-bold text-gray-800">{task.location}</p>
                                           <div className="flex items-center gap-2 mt-2">
                                               <img src={task.lister.avatar} alt={task.lister.name} className="w-6 h-6 rounded-full" />
                                               <div>
                                       ...
                                       {/* In a real app, this would open the verification camera flow */}
                                       <button className="w-full sm:w-auto mt-2 sm:mt-0 px-4 py-2 bg-green-600 text-white rounded-full font-semibold text-sm flex items-center justify-center gap-2 hover:bg-green-700">
