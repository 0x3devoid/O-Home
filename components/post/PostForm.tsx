...
export interface NewPropertyData {
    ...
    beds?: number;
    baths?: number;
    type?: 'rent' | 'sale' | 'business';
    businessType?: 'hotel' | 'shortlet' | 'office' | 'agency';
    latitude?: number;
    longitude?: number;
    needsAgent?: boolean;
...
const PostForm: React.FC<PostFormProps> = ({ onSubmit, initialData, currentUser, onAddStory, onCancel, onPostSuccess }) => {
  ...
  const [listingType, setListingType] = useState<'rent' | 'sale' | 'business'>('rent');
  const [businessType, setBusinessType] = useState<'hotel' | 'shortlet' | 'office' | 'agency'>('shortlet');
  const [serviceType, setServiceType] = useState<'general' | 'painting' | 'decorating' | 'cleaning' | 'maintenance' | 'landscaping' | 'renovation' | 'other'>('general');
  ...
  const [priceInterval, setPriceInterval] = useState<'year' | 'month' | 'week' | 'day' | 'night' | 'hour'>('year');
  ...
  // FIX: Added useEffect to clean up object URLs and prevent memory leaks.
  useEffect(() => {
    return () => {
      ...
      (position) => {
        ...
        setTimeout(() => {
            const addresses = [
                ...
                "Ozumba Mbadiwe Avenue, Victoria Island",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        await onSubmit(data);
        setImages([]); setImagePreviews([]); setVideos([]); setVideoPreviews([]); setDescription(''); setLocation(''); setPrice(''); setBeds(1); setBaths(1); setListingType('rent'); setPostType('property'); setNeedsAgent(false); setBusinessType('shortlet'); setAvailableRooms(1); setTotalRooms(1); setAmenities([]); setCheckInTime('14:00'); setCheckOutTime('11:00'); setMinimumStay(1); setBookingType('request'); setServiceType('general'); setLatitude(undefined); setLongitude(undefined); setCoordsCaptured(false);
        onPostSuccess();
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="flex items-center gap-2 mb-2">
        ...
        <button type="button" onClick={() => setPostType('text')} className={`px-3 py-1 text-sm font-semibold rounded-full transition-colors ${postType === 'text' ? 'bg-violet-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Text Post</button>
      </div>
      ...
      {postType === 'text' && (
        <div className="mt-3 p-3 bg-purple-50 border border-purple-200 rounded-lg">
          <label className="block text-sm font-medium text-purple-900 mb-2">Service Type (Optional)</label>
          <select
            value={serviceType}
            onChange={(e) => setServiceType(e.target.value as any)}
            className="w-full p-2 bg-gray-100/50 border-none rounded-md focus:outline-none text-sm"
          >
            <option value="general">General Post</option>
            <option value="painting">Painting Services</option>
            <option value="decorating">Interior Decorating</option>
          ...
          <p className="text-xs text-purple-700 mt-1">
            Share your services with the community. Perfect for showcasing your work through videos and images.
          </p>
        </div>
      )}
        ...
        <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={needsAgent}
              onChange={(e) => setNeedsAgent(e.target.checked)}
              className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
            />
            <div className="flex-1">
              <p className="text-sm font-medium text-blue-900">Need an agent to verify this property?</p>
              <p className="text-xs text-blue-700 mt-1">
                Agent verification adds credibility and helps with tours. Only verified agents can verify properties.
              </p>
            </div>
          </label>
        </div>
      )}
      {postType === 'property' && listingType === 'business' && (
        <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg space-y-3">
          <h4 className="text-sm font-semibold text-green-900">Business Booking Details</h4>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => setAvailableRooms(Math.max(1, parseInt(e.target.value) || 1))}
                className="w-full p-2 bg-gray-100/50 border-none rounded-md focus:outline-none text-sm"
                min="1"
              />
            </div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Minimum Stay (nights)</label>
            <input
              type="number"
              value={minimumStay}
              onChange={(e) => setMinimumStay(Math.max(1, parseInt(e.target.value) || 1))}
              className="w-full p-2 bg-gray-100/50 border-none rounded-md focus:outline-none text-sm"
              min="1"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setAmenities(e.target.value.split(',').map(a => a.trim()).filter(a => a))}
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
          {mediaPreviews.map(media => (
            <div key={media.id} className="relative group aspect-square">
              ...
              <button 
                type="button" 
                onClick={() => media.type === 'image' ? removeImage(media.id) : removeVideo(media.id)} 
                className="absolute top-1.5 right-1.5 w-5 h-5 bg-black/60 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/80"
              > 
      ...
      <div className="flex flex-wrap justify-between items-center mt-3 gap-y-2">
        <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
            ...
            <button 
                ...
                className="flex items-center gap-2 text-violet-600 p-2 rounded-full hover:bg-violet-100"
                title="Add to your story"
            >
                <CameraIcon className="w-6 h-6" />
                <span className="text-sm font-semibold hidden sm:inline">Add Story</span>
            </button>
            <input 
                ref={storyInputRef} 
                type="file" 
            ...
            <>
                <div className="flex items-center gap-1">
                    ...
                    <button type="button" onClick={() => setListingType('sale')} className={`px-2 py-1 text-xs font-semibold transition-colors ${listingType === 'sale' ? 'bg-violet-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Sale</button>
                    {currentUser.accountType === 'business' && currentUser.businessStatus === 'verified' && (
                        <button type="button" onClick={() => setListingType('business')} className={`px-2 py-1 text-xs font-semibold rounded-r-md transition-colors ${listingType === 'business' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Business</button>
                    )}
                </div>
                ...
                {listingType === 'business' && (
                    <select value={businessType} onChange={e => setBusinessType(e.target.value as any)} className="text-xs font-semibold bg-gray-200 text-gray-700 rounded-md p-1 border-none focus:ring-1 focus:ring-green-500">
        ...
        <div className="flex items-center gap-2">
            <button type="button" onClick={onCancel} className="px-6 py-2 text-gray-700 font-semibold rounded-full hover:bg-gray-100 transition-colors">
                Cancel
            </button>
            <button type="submit" disabled={!isFormValid || isSubmitting} className="px-6 py-2 bg-violet-500 text-white rounded-full font-semibold hover:bg-violet-600 disabled:bg-violet-300 flex items-center justify-center min-w-[80px]">
                {isSubmitting ? <SpinnerIcon className="w-5 h-5" /> : 'Post'}
            </button>
        </div>
      </div>
    </form>
