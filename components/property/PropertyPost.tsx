...
const PropertyPost: React.FC<PropertyPostProps> = (props) => {
  const { property, currentUser, onViewComments, onStartVerification, onMessageLister, onViewProfile, onShareToTeam, teams, onViewOnMap, onOpenDeleteModal, onBlockUser } = props;
  const { lister, price, location, beds, baths, images, videos, verificationStatus, verifier, description, type, postType, likes, comments, timestamp } = property;
  ...
  return (
    <article className="bg-white">
      ...
      <div className="p-3">
          ...
          {likeCount > 0 && <p className="text-sm font-bold mt-2">{likeCount.toLocaleString()} {likeCount === 1 ? 'like' : 'likes'}</p>}
          ...
          {postType === 'property' && (
              <div className="mt-2">
                <p className="text-base font-bold text-violet-700">
                  {formatPrice(price!, type, property.priceInterval)}
                </p>
